...
const productosFiltrados = computed(() => {
  ...
  return productos.value.filter(p => p.tipo === filtroActual.value)
})

// Calcular tipos disponibles
const tiposDisponibles = computed(() => {
  const tipos = new Set<string>()
  productos.value.forEach(p => tipos.add(p.tipo))
...
const totalItems = computed(() => {
  return Object.values(pedido.value).reduce((sum, cantidad) => sum + cantidad, 0)
})

// Calcular total del pedido
const totalPedido = computed(() => {
  let total = pedidoItems.value.reduce((sum, item) => sum + (item.precio * item.cantidad), 0)
