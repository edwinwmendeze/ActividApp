<!-- components/Admin/PedidosList.vue -->
<template>
    <div class="pedidos-list">
      ...
      <div class="list-header">
        ...
        <div class="filters-bar">
          ...
          <div class="filter-options">
            ...
            <select v-model="filtros.colaborador" @change="filtrarPedidos">
            ...
            <button @click="limpiarFiltros" class="action-button secondary small">
              Limpiar filtros
            </button>
          </div>
        </div>
      ...
      <div v-else-if="pedidosFiltrados.length === 0" class="empty-state">
        ...
        <p v-else>
          Aún no hay pedidos registrados para esta actividad.
        </p>
      </div>
        ...
        <table class="pedidos-table">
          ...
          <tbody>
            <tr v-for="pedido in pedidosFiltrados" :key="pedido.id" :class="{ 'entregado': pedido.estado === 'entregado' }">
              ...
              <td>{{ obtenerNombreColaborador(pedido.colaborador_id) }}</td>
              <td>
  ...
  const pedidosFiltrados = ref<any[]>([])
  ...
  const obtenerNombreColaborador = (colaboradorId?: string) => {
    if (!colaboradorId) return 'No asignado'
    
    const colaborador = colaboradores.value.find(c => c.id === colaboradorId)
    return colaborador ? colaborador.nombre : 'Desconocido'
  }
  
  // Cargar pedidos y colaboradores
  const cargarDatos = async () => {
    ...
    try {
      ...
      if (colaboradoresError) throw new Error(`Error al cargar colaboradores: ${colaboradoresError.message}`)
      colaboradores.value = colaboradoresData || []
      ...
      if (pedidosError) throw new Error(`Error al cargar pedidos: ${pedidosError.message}`)
  ...
  const filtrarPedidos = () => {
    pedidosFiltrados.value = pedidos.value.filter(pedido => {
      ...
      const matchColaborador = !filtros.value.colaborador || pedido.colaborador_id === filtros.value.colaborador;
      ...
      if (filtros.value.fechaDesde) {
        ...
        matchFechaDesde = fechaPedido >= fechaDesde;
      }
      ...
      if (filtros.value.fechaHasta) {
        ...
        matchFechaHasta = fechaPedido <= fechaHasta;
      }
      
      return matchBusqueda && matchEstado && matchPago && matchColaborador && matchFechaDesde && matchFechaHasta;
    });
  }
  ...
  const verDetallesPedido = async (pedido: any) => {
  ...
  const editarEstadoPedido = (pedido: any) => {
    pedidoSeleccionado.value = pedido;
  ...
  const actualizarEstadoPedido = (estado: string) => {
  ...
  // Marcar como pagado
  const marcarComoPagado = async (pedido: any) => {
